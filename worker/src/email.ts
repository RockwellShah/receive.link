...
export async function sendDropLinkEmail(env: Env, to: string, dropUrl: string, manageUrl: string, label: string): Promise<void> {
  const subject = `Your link is ready${label ? ` · ${label}` : ""}`;
  const text =
    `Your link${label ? ` ("${label}")` : ""} is ready.\n\n` +
    `Share it so anyone can send you files. Whatever they drop is end-to-end encrypted to your passkey:\n${dropUrl}\n\n` +
    `Keep this private link to turn it off later. Don't share it:\n${manageUrl}\n\n` +
...
export async function sendDownloadEmail(env: Env, to: string, downloadUrl: string, label: string, manageUrl?: string): Promise<void> {
  ...
  const text =
    `Someone sent a file or folder to your link${label ? ` ("${label}")` : ""}.\n\n` +
    `Open it on receive.link (you'll need your passkey):\n${downloadUrl}\n\n` +
