...
export async function sendConfirmEmail(env: Env, to: string, confirmUrl: string, label: string): Promise<void> {
  ...
  await env.EMAIL.send({ to, from: env.MAIL_FROM, subject, text });
}

/** Delivery email: someone dropped a file; here is the link to open it. */
export async function sendDownloadEmail(env: Env, to: string, downloadUrl: string, label: string): Promise<void> {
  const subject = `A file was sent to your FileKey Drop${label ? ` · ${label}` : ""}`;
  const text =
    `Someone dropped a file for you${label ? ` ("${label}")` : ""}.\n\n` +
    `Open it in FileKey (you'll need your passkey):\n${downloadUrl}\n\n` +
    `This link expires in 7 days. We can't read your files, and we don't store your address.\n`;
  await env.EMAIL.send({ to, from: env.MAIL_FROM, subject, text });
}
